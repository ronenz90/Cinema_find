...
def scrape_showtimes(watch: Watch, headless: bool = True) -> Dict[str, List[str]]:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            previous_hours_snapshot = None
            for date_str in dates:
                page.evaluate("(d) => window.ticketsVM.selected.date(d)", date_str)
                hours = []
                changed = False
                for attempt in range(8):
                    page.wait_for_timeout(800)  # give the AJAX refresh time before every read
                    hours = page.evaluate(
                        """
                        () => {
                            const vm = window.ticketsVM;
                    ...
                    if hours and hours != previous_hours_snapshot:
                        changed = True
                        break
                if hours and not changed:
                    # Never became different from the previous date's hours even
                    # after generous retrying. Could be a genuine coincidence
                    # (same showtime every day) or a stale read - we can't tell
                    # for certain, so we keep the data but flag it for visibility
                    # rather than silently dropping a possibly-real showtime.
                    log.warning(
                        "Hours for %s are identical to the previous date after retrying - "
                        "keeping them, but this could be a stale read rather than a real coincidence.",
                        date_str,
                    )
                if hours:
                    previous_hours_snapshot = hours
